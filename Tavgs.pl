...
GetOptions ('delete' => \$delete,'adapt=s{3}' => \@hmmer, 'exon' => \$exon,'number:i' => \$number_of_genes,'length:i' => \$length,'file=s' => \$protein_file,'protein=s' => \$protein,'help|?' => \$help,'out=s' => \$out_file,'blast_protein=s{5}' => \@blast,'compact' => \$one_file,'blast_file=s{4}' => \@blast_file,'make_cluster_db:s' => \$db_name,'cluster=s{2}' => \@cluster);
...
sub make_file			# The main sub for making the files which willbe used by Tavgs_display.pl. Different options will enable different files here.
  ...
  for my $list (@proteins)
	...
	if ($column[3]>$start-$length and $column[4]<$end+$length)
	  ...
	  $column[8]=~/ID=([\w.]*)[;,]/;
	  my $ID=$1;
	  print $OUT join("\t",$column[0],$column[3],$column[4],$column[6],$ID,$column[2]),"\n";
	  }
	}
      }
